...
function getUser(req, res, next){
	...
	// let fileName = path.join(".", req.app.locals.config.userDir, id + ".json");
...
function createHTML(users, req){
	...
	users.forEach(user => {
		result += `<a href="${req.app.locals.config.host}/${req.app.locals.config.userDir}/${user.id}">${user.name}</a><br>`;
	});

	//Add next and previous page links
	if(req.query.page > 1){
		result += `<a href="${req.app.locals.config.host}/${req.app.locals.config.userDir}?page=${req.query.page - 1}&name=${req.query.name}">Previous<a><br>`;
	}
	result += `<a href="${req.app.locals.config.host}/${req.app.locals.config.userDir}?page=${req.query.page + 1}&name=${req.query.name}">Next<a><br>`;
